...
Cobogó foundations define **semantic roles and compositional invariants before concrete values**. A consumer may supply its own values when the role remains legible and accessible.
...
- `surface` — grouped or elevated region when grouping is actually required;
...
Consumers may map those roles to different concrete scales by density/context. A dense admin surface and an editorial article should not be forced onto one pixel scale.
...
State must never depend on color alone. Focus, selected, warning, invalid, destructive, disabled, loading and stale/partial data states need textual, structural, iconographic or behavioral redundancy appropriate to context.
...
This proposal is currently pressured by three real consumer clusters:

1. **Public analytical/data reading** — CausaGanha, Ficha, Baliza Site.
2. **Dense review/audit** — Astronauta, Sisprev.
3. **Editorial/provenance** — O Vigia, Quem São Eles?.

A role should not be promoted to stable core merely because one consumer has a token for it. Reuse across distinct contexts is stronger evidence.
