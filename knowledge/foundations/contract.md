...
3. **Editorial/publication** — O Vigia, Quem São Eles?, Franklin blog, The Theater, The Lab, Rationality Club LatAm, Adi blog.
4. **Authoring/preview tools** — Intuit, Formato Legal.
5. **Paginated document/print** — Formato Legal, Sisprev.
6. **Immersive/game interaction** — Aleph the Game as experimental pressure evidence for motion and expressive interaction state.
7. **Self-dogfood** — Cobogó itself must consume applicable public capabilities without privileged APIs.

A role should not be promoted to stable core merely because one consumer has a token for it. Reuse across distinct contexts is stronger evidence; platform/accessibility invariants can justify promotion independently. One novel consumer can justify experimentation without automatically creating a stable shared pattern. Cobogó's own preferences do not count as extra external evidence.