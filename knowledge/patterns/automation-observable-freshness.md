...
- **Funes Memories** is a registered Cobogó consumer whose home previously used a pulsing `SYSTEM LIVE` label even though the repo could prove only a published archive and the dates of materialized memories. [funes-memories#27](https://github.com/franklinbaldo/funes-memories/pull/27) replaces the operational-sounding claim with factual archive status, derives the latest memory from the content collection and explicitly states that publication availability does not imply current agent activity. [funes-memories#29](https://github.com/franklinbaldo/funes-memories/pull/29) then adds controlled Chromium evidence against the real Project Pages base path. Run `32109957097` rendered `PUBLIC ARCHIVE`, the latest-memory `<time>`, the operational-uncertainty sentence and a real chronicle article with zero page errors, uploading home/article screenshots plus structured capture state. This is positive **rendered-browser evidence** of a deliberately non-operational presentation.
...
Before considering stable promotion, require independently verified rendered evidence from at least two materially different positive implementations, backed by observable publication/execution evidence and retaining distinct presentation. Funes Memories now supplies one such rendered implementation. Rationality Club LatAm remains the strongest candidate for the second, but its current evidence should not be upgraded until the merged public surface is actually deployed and captured. Negative evidence remains useful if a project correctly chooses to suppress or qualify automation claims instead of exposing operational state.
