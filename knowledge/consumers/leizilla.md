...
- GitHub Pages/deploy — presente; a #163 foi publicada no commit `b5afe794073fc5645c3fb4edacc8146bf75976df` pela execução `33770975164`.
- Verificação visual reproduzível — desktop `1280×900` e celular `390×844`, agora também com falha controlada de acesso ao dataset. Em `main`, a execução `33770975121` produziu o arquivo `9899626172` ligado ao mesmo commit `b5afe794...`.
...
A capacidade nova de verificação controlada foi encaminhada ao workspace na issue `franklinbaldo/workspace#35`; este cartão não edita o `ProjectProfile`.
...
- `visual-evidence-as-quality-gate` — **stable**, isto é, já funcionou de forma convergente em pelo menos dois projetos.
...
A avaliação atual é **20 de 24 pontos — faixa boa**: Clareza 4, Explicabilidade 4, Autonomia 4, UX 3, UI 2 e Confiança 3.

A dívida material restante é `leizilla#162`: em `390×844`, os filtros de ente e tipo de norma ficam estreitos a ponto de perder o texto que os diferencia. Isso mantém UI em 2/4.

A antiga dívida `leizilla#161` foi resolvida pela #163. O estado de falha agora comunica indisponibilidade observada, não ausência presumida do acervo.
