...
- `dinossauro` — **plan-only civil-tech project, not a current direct Cobogó consumer on 2026-08-16 evidence**. Current `main` contains only lightweight project docs plus a detailed pipeline plan; no Astro/frontend implementation, data pipeline, workflow, Pages deployment or rendered product surface exists yet. The plan explicitly proposes a future Astro/Cobogó web layer and Parquet/Internet Archive pipeline, but these are roadmap claims, not operational capabilities or adoption evidence. Reconsider when PR 0+ materializes an actual human-facing surface; until then, keep the planned Baliza/Cobogó/CausaGanha relationships as architectural intent rather than consumer proof.
- `nanochat` — **upstream fork, not a current Franklin-owned Cobogó consumer on 2026-08-16 evidence**. GitHub identifies `franklinbaldo/nanochat` as a fork of `karpathy/nanochat`; it has no Pages surface, no Franklin-owned deployment/homepage, and the README/UI instructions are upstream material pointing readers to Karpathy's hosted nanochat instance. The fork does contain upstream web-serving code, but that is not evidence of a Franklin product surface. Reconsider only if this fork develops material Franklin-owned UI/product behavior or deployment.
- `life-of-menard` — **concept/README artifact, not a current direct Cobogó consumer on 2026-08-16 evidence**. The README describes an ambitious branch-and-duel experiment over a Hrönir-derived engine, including CLI modules, data, docs and tests, but the current repository tree contains only `.gitignore`, `LICENSE` and `README.md`. No owned application, CLI implementation, workflow, generated publication or deploy surface is present in the current branch. Treat the README as a product concept/history artifact rather than evidence that the described engine or a human-facing interface currently exists. Reconsider if executable/product artifacts reappear.
